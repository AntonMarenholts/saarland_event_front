...
export default function EventDetailPage() {
  ...
  const handleSetReminder = async () => {
    ...
    try {
      ...
      alert(`${t('reminder_set_alert')} ${remindAtDate.toLocaleString(i18n.language)}`);
      ...
      <article className="bg-gray-800 rounded-lg overflow-hidden shadow-lg">
        ...
        <div className="p-8">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-2 text-gray-400 mb-6 border-b border-gray-700 pb-4">
            {/* <-- 4. ПЕРЕВОДИМ СТАТИЧНЫЙ ТЕКСТ --> */}
            <p><strong>{t('city_label')}:</strong> {event.city.name}</p>
            <p><strong>{t('category_label')}:</strong> {event.category.name}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-8 rounded-lg shadow-xl text-white">
            ...
            <div className="flex justify-end gap-4">
              <button onClick={() => setShowReminderModal(false)} className="px-4 py-2 rounded bg-gray-600 hover:bg-gray-500">{t('cancel_button')}</button>
              <button onClick={handleSetReminder} className="px-4 py-2 rounded bg-teal-600 hover:bg-teal-700">{t('set_button')}</button>
            </div>
          </div>
        </div>
      )}
