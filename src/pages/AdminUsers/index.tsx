    ...
    <div className="w-full text-white">
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        <ul className="space-y-3">
          {users.map(user => (
            <li key={user.id} className="flex justify-between items-center bg-gray-700 p-3 rounded">
              <div>
                <p className="font-bold">{user.username} ({user.role})</p>
                <p className="text-sm text-gray-400">{user.email}</p>
              </div>
              {user.role !== 'ROLE_ADMIN' && (
                <button onClick={() => handleDelete(user.id)} className="text-red-500 hover:text-red-400 text-sm">{t('delete')}</button>
              )}
            </li>
          ))}
        </ul>
